...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(Column("Field2", Integer).like("%abc"), """["Field2"] endswith_cs 'abc'"""),
        pytest.param(
...
def test_function_text(f: str, expected: str):
    ...
    active_users_col = literal_column("ActiveUsers").label("ActiveUserMetric")
    query = select([event_col, active_users_col]).select_from(text("ActiveUsersLastMonth"))
    query_compiled = str(query.compile(engine, compile_kwargs={"literal_binds": True})).replace("\n", "")
    query_expected = (
        '["ActiveUsersLastMonth"]'
        '| extend ["ActiveUserMetric"] = ["ActiveUsers"], '
        '["EventInfo_Time"] = ' + expected + '| project ["EventInfo_Time"], ["ActiveUserMetric"]'
    )
    assert query_compiled == query_expected
...
def test_percentile_by_text():
    ...
    # raw query text from query
    query_expected = (
        '["SalesData"]' '| summarize ["Measure 1"] = percentile(["quantity_ordered"], 99) ' '| project ["Measure 1"]'
    )
    assert query_compiled == query_expected


def test_dcountif_by_text():
    event_col = literal_column("dcountif(year, city == 'Paris' or city in ('Madrid'))").label("Measure 1")
    query = select(
        [
            event_col,
        ]
...
def test_countif_by_text():
    event_col = literal_column("countif(city == 'Paris' OR city in ('Madrid'))").label("Measure 1")
    query = select(
        [
            event_col,
        ]
