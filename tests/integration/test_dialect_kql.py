...
from sqlalchemy import Column, MetaData, String, Table, create_engine, func, literal_column, text
...
def test_date_bin_ops(test_label, group_fn, temp_table_name, expected, compare_dates):
    ...
    with kql_engine.connect() as connection:
        ...
        expected_records = (
            {(expected, (now - timedelta(days=i)).strftime("%Y-%m-%dT%H:%M:%SZ")) for i in range(9, 0, -1)}
            if compare_dates
            else {expected}
        )
