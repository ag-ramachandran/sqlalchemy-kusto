import logging
import re
from typing import Dict, List, Optional, Tuple  # add for typing
...
class KustoKqlCompiler(compiler.SQLCompiler):
    ...
    def visit_select(
        ...
        **kwargs,
        ...
        if hasattr(from_object, "element"):
            ...
            compiled_query_lines.append(self._convert_schema_in_statement(from_object.left.fullname))
        else:
        ...
        if select_stmt._whereclause is not None:
            ...
            if where_clause:
                ...
                converted_where_clause = self._sql_to_kql_where(where_clause_reformatted)
        ...
        if select_stmt._limit_clause is not None:
            ...
            compiled_query_lines.append(f"| take {self.process(select_stmt._limit_clause, **kwargs)}")
    ...
    def _legacy_join(self, select_stmt: selectable.Select, **kwargs):
        ...
        for right, on_columns, _left, flags in select_stmt._legacy_setup_joins:
            ...
            if on_columns is not None:
                left_clause_col, _ = self._extract_column_name_and_alias(on_columns.left)
                right_clause_col, _ = self._extract_column_name_and_alias(on_columns.right)
                on_clause = (
    ...
    def _extract_maybe_agg_column_parts(column_name) -> Optional[str]:
        ...
        if match_agg_cols and match_agg_cols.groups():
            ...
            aggregate_func, distinct_keyword, agg_column_name, extra_params = match_agg_cols.groups()
    ...
    @staticmethod
    def _escape_and_quote_columns(name: Optional[str], is_alias=False) -> str:
        if name is None:
            return ""
        if (KustoKqlCompiler._is_kql_function(name) or KustoKqlCompiler._is_number_literal(name)) and not is_alias:
            return name
    ...
    def _sql_to_kql_where(where_clause: str) -> str:
        ...
        where_clause = re.sub(r"(?<=[^=])=(?=\s|$|[^=])", r"==", where_clause, flags=re.IGNORECASE)
        ...
        where_clause = where_clause.replace("<==", "<=")
        where_clause = where_clause.replace("<>", "!=")
        where_clause = re.sub(r"(\s|^)lower\(", r"\1tolower(", where_clause, flags=re.IGNORECASE)

        where_clause = re.sub(r"(\s)(<>|!=)\s*", r" \2 ", where_clause, flags=re.IGNORECASE)  # Handle '!=' and '<>'
        where_clause = re.sub(
    ...
    def _sql_to_kql_aggregate(
        ...
        if sql_to_kql_aggregate_function:
            ...
            return_value = f"{aggregation_function}({column_name_escaped}{extra_params})"
        return return_value
